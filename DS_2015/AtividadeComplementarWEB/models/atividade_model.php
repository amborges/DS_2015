...
class AtividadeModel extends BaseModel {
    ...
    public function getResumo($matricula, $seqAtividade){
			$sql = "SELECT * FROM " . $this->table_name . " WHERE `matricula` = '$matricula' AND `seqAtividade` = '$seqAtividade';";
    	...
    	$this->create_connection();
		  $result = $this->conn->query($sql);
			...
			$rows = NULL;
			$str = "Falha na requisição";
			if($result->num_rows > 0){
				$rows = $result->fetch_assoc();
				$str = "A atividade com a descrição \"".$rows['descricaoAtividade']."\", pertencente ao aluno sob matricula \"".$rows['matricula']."\", será rejeitado pois...";
			}
			return $str;
    }
    ...
    public function update($listaDeAtividades){
    	...
    	public function update_professor($la){
    	//Eu queria unificar os updates, mas infelizmente, vai ter q ser um a um
